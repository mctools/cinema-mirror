...
d2o_qtrue, d2o_s = d2o.fillHist(1, 85, xmin=0.1, xmax=120.0, numbin=150, linear=False)
v = SQTrueHist('/data/yangni/cinemaRunData/2022_8_7/HW_D_1_NumBig/V_R1/*ScororNeutronSq_SofQ_He_seed*.h5')
v_qtrue, v_s = v.fillHist(1, 85, xmin=0.1, xmax=120.0, numbin=150, linear=False)
...
d2oMS1_qtrue, d2oMS1_s = d2oMS.sqHist(1, 85, xmin=0.1, xmax=120.0, numbin=150, scatnum=1, inelastic=True, linear=False)
d2oMS1_s_calib = np.divide(d2oMS1_s/d2o_atomNum/numNeutron, v_s/v_atomNum/numNeutron, where=v_s!=0.)
d2oMS2_qtrue, d2oMS2_s = d2oMS.sqHist(1, 85, xmin=0.1, xmax=120.0, numbin=150, scatnum=2, inelastic=True, linear=False)
d2oMS2_s_calib = np.divide(d2oMS2_s/d2o_atomNum/numNeutron, v_s/v_atomNum/numNeutron, where=v_s!=0.)
d2oMS3_qtrue, d2oMS3_s = d2oMS.sqHist(1, 85, xmin=0.1, xmax=120.0, numbin=150, scatnum=3, inelastic=True, linear=False)
d2oMS3_s_calib = np.divide(d2oMS3_s/d2o_atomNum/numNeutron, v_s/v_atomNum/numNeutron, where=v_s!=0.)
probability = d2oMS.probaMultiScat(1, 85, scatnum=2) # calculate the probability of  scattering number = 2
print(probability)
...
# plt.xlim(0.0, 16.0)
# plt.ylim(1e-4, 1e+1)
# plt.semilogy(d2o_qtrue, d2o_s_calib, label=f'R_D$_2$O=1mm, b=1, Total ')
# plt.semilogy(d2oMS1_qtrue, d2oMS1_s_calib, label=f'R_D$_2$O=1mm, b=1, 1Scattering qtrue')
# plt.semilogy(d2oMS2_qtrue, d2oMS2_s_calib, label=f'R_D$_2$O=1mm, b=1, 2Scatterings')
# plt.semilogy(d2oMS3_qtrue, d2oMS3_s_calib, label=f'R_D$_2$O=1mm, b=1, 3Scatterings')
# plt.xlabel('Q[Å$^-$$^1$]')
# plt.ylabel('S[/incident particle/atom]')
# plt.grid()
# plt.legend(fontsize=12*1.5)
