...
const Prompt::SampledResult& Prompt::ModelCollection::pickAndSample(double ekin, const Vector &dir) const
{
  ...
  {
    //fixme:!!
    printf("WARNING, if this message is shown from Prompt::ModelCollection::pickAndSample. The event is sampled with different incident energy and/or direction other than those used for calculating cross sections\n");
    return m_res;
  }
  ...
  if(m_models[i]->isOriented())
  {
    ...
    m_res.final_dir =  activeVolume.getGeoTranslator().local2Global_direction(res.final_dir);
    m_res.final_ekin = res.final_ekin;
  }
  else {
    auto res = m_models[i]->sampleReaction(ekin, dir);
    m_res.final_dir =  res.final_dir;
    m_res.final_ekin = res.final_ekin;
  }
