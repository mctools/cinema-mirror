...
f = PromptFileReader('ScorerPSD_NeutronHistMap_seed5.mcpl.gz')
...
count=count.sum()
integral= data.sum()
plt.title(f'ScorerPSD_NeutronHistMap_seed5, integral {integral}, count {count}')
plt.figure()
plt.subplot(211)
plt.plot(x[:-1]+np.diff(x)*0.5, data.sum(axis=1))
plt.xlabel('integral x')
plt.title('ScorerPSD_NeutronHistMap_seed5')
plt.subplot(212)
